# Spec: llm-model-manifest

## Overview

Defines the extended model manifest schema that supports both ASR and LLM model entries. Introduces `category`, `has_thinking_mode`, and `thinking_disable_token` fields to all manifest entries, and specifies the required LLM GGUF model entries that back the hardware recommendation decision tree.
