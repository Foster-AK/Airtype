...
Defines the behavior of clipboard-based text injection for Airtype's voice input workflow. The system writes recognized text to the clipboard, simulates a paste keystroke into the target application, then restores the original clipboard content, ensuring a seamless and non-destructive injection experience across Windows, macOS, and Linux.
